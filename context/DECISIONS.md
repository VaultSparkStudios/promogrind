...
### 2026-05-18 — Replace IGNIS HTTP placeholder with real CLI invocation (S93)

- Status: shipped
- Context: `scripts/lib/ignis-rank.mjs` was scaffolded in S79 against a hypothetical `IGNIS_MCP_URL` HTTP endpoint that was never implemented. IGNIS only ships stdio MCP + a CLI. Result: every consumer-repo `genius-list` refresh silently reported `IGNIS source: fallback` and never used live intelligence.
- Decision: replace the HTTP placeholder with a `spawnSync` invocation of `<IGNIS_ROOT>/dist/cli.js export json <projectRoot>` (caches `json/ignis/output/export.json` for 1h), then layer a pillar-aware boost over the existing deterministic per-item ranking via a `category → pillar` map (SECURITY→vitality, LAUNCH→potential, etc.). Auto-resolves sibling repo at `../vaultspark-ignis`; `IGNIS_ROOT` env override; `IGNIS_DISABLE=1` to force fallback. Legacy HTTP path kept as a future option.
