...
## Now
- [ ] [SIL] Wire topPlaybook into TodayDashboardPanel → getNextBestAction call — `matchPlaybooks(appData, context)` result needs to be passed as `topPlaybook` from TodayDashboardPanel so matched playbooks actually appear as the dashboard next-best-action card
- [ ] [SIL] Playbook CTA card in dashboard UI — add a dedicated render format for playbook candidates (playbook icon, step count, bankroll-fit reason) so "Try: Bonus Bet Conversion" renders distinctly from generic action prompts
