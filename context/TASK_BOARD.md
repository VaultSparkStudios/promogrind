...
## Now
- [ ] [SIL] Add component tests for extracted calculators (BonusBet, KellyCriterion) — component-level render/interaction tests for the two most complex extracted calculators; currently only math helpers are tested, not render logic
- [ ] [SIL] Wire getDashboardSnapshot topPlaybook into operator briefing — `src/operator/briefing.js` / `DailyBriefPage.jsx` should consume `topPlaybook` from the snapshot so the operator command brief can surface the top matched playbook as part of the daily action plan
