...
- [ ] [SIL] Entity-aware sync continuation — move from mirrored entity tables to finer-grained conflict handling inside ledger/workflow domains, then reduce the legacy `promogrind_data` row to a compatibility layer — **S54 progress**: `src/sync.js` now merges ledger rows, workflow inbox rows, result-feedback rows, and workflow-history rows per record across local + remote state instead of replacing whole entity arrays; reducing `promogrind_data` and deepening tracker-domain conflict rules remain open
...
1. [ ] [SIL] Entity-aware sync continuation — now partially advanced in S54; finish legacy blob reduction plus deeper tracker-domain merge rules
2. [ ] Canonical Promo Operating Graph — unify scoring/policy across dashboard, Track, AI, sync, and Studio surfaces
3. [x] Bundle trim after sync tranche — **DONE S54**: moved `PromoWalkthrough` behind a lazy chunk so the main bundle dropped back to `~415.9KB`, restoring headroom under the 420KB cap after the sync merge work
4. [ ] Offline-first ledger queue — queue writes, show sync status, and resolve conflicts per entity timestamp
5. [ ] Playbooks — reusable promo routines by bankroll, promo type, and available books
6. [ ] Observability dashboard — activation, return rate, CTA CTR, AI usage, and monetization health
7. [ ] Community intel upgrade — freshness, verification, report quality, and region filters on promo submissions
8. [ ] Keyboard-nav follow-through — extend beyond tab bars to pinned favorites, compare selector flows, and remaining dialog/button clusters
9. [ ] Motion-reduce guard for transitions (`prefers-reduced-motion`)
10. [ ] Bundle budget in CI — fail or warn when first-load bundle exceeds target
...
- [ ] [SIL] Entity-aware sync continuation — move from mirrored entity tables to finer-grained conflict handling inside ledger/workflow domains, then reduce the legacy `promogrind_data` row to a compatibility layer — **S54 progress**: sync now preserves per-record ledger/workflow/history changes across devices instead of whole-array last-write wins, but the legacy blob is still a compatibility mirror and tracker-domain conflict rules still need the same treatment
