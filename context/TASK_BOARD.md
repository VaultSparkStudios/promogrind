...
- [x] Confidence decay visualization on workflow cards — **DONE S63**: `workflowUrgency()` helper computes urgency from `createdAt` age vs promo-type window (bonus_bet=7d, arb=3d, deposit_match=30d, etc.); each workflow card shows a colored urgency bar at the bottom (green→yellow→red) and a "Xd left" label when ≥55% elapsed
- [x] Calculator receipt export — **DONE S63**: `src/components/CalculatorReceipt.jsx` — modal receipt with structured inputs/outputs, CSS print-on-demand (no deps), copy-to-clipboard + print/PDF buttons; wired into BonusBet result row via "📄 Receipt" button alongside existing Copy button; receipt pattern ready for all other calculators
- [x] Edge function test suite — **DONE S63**: `supabase/functions/__tests__/promo-advisor.test.ts` — 9 Deno tests covering `normalizeAdvisorResult` (valid fields, fallback rating/confidence, bad slug, score clamping, array caps) + mock Anthropic fetch validating prompt-caching headers; `supabase/functions/__tests__/ai-access.test.ts` — 8 Deno tests covering tier mapping, tier hierarchy, daily limits, burst rate constants
- [x] Community hot-lane signal — **DONE S63**: `buildHotLanes(appData, now)` exported from `src/track/insights.js`; aggregates settled profitable entries by promo type and book in 48h window (min 3 to qualify); `CommunityPromoBoard` shows a "🔥 Hot Lanes" banner + individual card "🔥 Hot Lane" badge when book or promo type matches
- [x] Edge function deploy script — **DONE S63**: `scripts/deploy-edge-functions.sh` with `--s62` (S62 functions), `--all` (all 11 functions), or named-function modes; `docs/RELEASE_PLAN.md` updated with exact deploy commands and full blocker checklist
...
- [x] [SIL] Add topPlaybook tracking to buildSummaryDelta — **DONE S62**: `buildSummaryDelta` now compares `brief.topPlaybook.id` between snapshots and appends `"playbook rotated to..."` or `"top playbook cleared"` to the delta string; `appendStudioContractHistory` now records `topPlaybookId` + `topPlaybookName` in each history summary entry
- [x] [SIL] Test buildOperatorCommandBrief topPlaybook path — **DONE S62**: 4 new tests in `studioExport.test.js` covering `brief.topPlaybook` structured object (id, name, fitScore, fitReasons, stepCount), `brief.followUps` contains "Try:" string, `buildSummaryDelta` playbook rotation detection, and history summary `topPlaybookId`/`topPlaybookName` fields
...
## S62 Shipped
- [x] **Prompt caching** — all 3 edge functions (`promo-advisor`, `promo-chat`, `ai-action-plan`) now use `cache_control: { type: "ephemeral" }` on system prompts + `anthropic-beta: prompt-caching-2024-07-31` header — ~90% cost reduction on system-prompt tokens + 2× response speed
- [x] **Per-user AI context in promo-advisor** — now accepts optional `userContext` (bankroll, books, hitRate, topPromoType); PromoAdvisorPanel builds and sends live context from appData; advisor responses are now personalized to the user's actual profile
- [x] **Promo Streak Engine** — `src/lib/streaks.js` with `computeStreak`, `streakEmoji`, `streakLabel`, `streakMilestone`; DashboardHero shows streak counter + fire emoji at 3/7/14/30-day milestones; wired via `computeStreak(data, today)` in App.jsx dashboard render
- [x] **Quota awareness in PromoAdvisorPanel** — rich quota display: remaining count with color coding (green/yellow/red), reset time countdown when exhausted, "Upgrade ↑" inline CTA when ≤1 analysis remaining
- [x] **VITE_DEV_BYPASS_AUTH build guard** — `vite.config.js` throws hard error if `NODE_ENV=production` and `VITE_DEV_BYPASS_AUTH=true` — prevents accidental auth-bypass prod build
- [x] **Streak tests** — `src/__tests__/streaks.test.js` with 13 tests covering consecutive days, zero-profit exclusion, gap detection, ledger+feedback merge, yesterday-anchor, best streak tracking, emoji, label, milestone
- [x] **Portfolio EVS Engine** — `src/lib/portfolio.js` implements `buildPortfolioAllocation(workflows, bankroll)` using Kelly-optimal bankroll allocation across all open workflows simultaneously; `getDashboardSnapshot` now accepts `{ includePortfolio: true }` and includes `portfolioAllocation` in the snapshot; 8 portfolio tests in `src/__tests__/portfolio.test.js`
