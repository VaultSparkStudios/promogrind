...
- [ ] [SIL] Extract remaining large calculators from App.jsx — `TeaserCalc`, `RoundRobinCalc`, `ParlayBuilder`, `SGPEstimator`, `BetSizingAdvisor`, `LineShop` still inline; together ~30-40KB of potential bundle headroom recovery
- [ ] [SIL] Wire topPlaybook into LaunchCommandCenterPanel operator command brief — `buildCommandBrief` in the cockpit doesn't yet reference `topPlaybook`; the machine-usable daily brief text should also surface the matched playbook in the action plan narrative
