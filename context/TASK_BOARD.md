...
- [ ] Canonical Promo Operating Graph — unify promo, workflow, action, drift, confidence, and settlement policy into one shared decision model across dashboard, Track, AI, sync, and Studio surfaces; **S55 progress**: shared operating-action candidate generation and decision selection now drive dashboard next-best-action plus Studio priorities/briefs, but Track, AI, and sync policy still need the same canonical path

## Current Priority Order
1. [ ] Canonical Promo Operating Graph — extend the shared operating-action model into Track, AI, and sync policy so more surfaces stop scoring independently
2. [ ] Offline-first ledger queue — move from visible sync diagnostics to a durable IndexedDB-backed offline write path with entity-aware replay/conflict handling
3. [ ] Playbooks — reusable promo routines by bankroll, promo type, and available books
4. [ ] Move auth tokens to httpOnly cookies OR accept localStorage + add refresh-rotation test coverage for hijack scenarios
5. [ ] Keyboard-nav follow-through — extend beyond tab bars to pinned favorites, compare selector flows, and remaining dialog/button clusters
6. [ ] Motion-reduce guard for transitions (`prefers-reduced-motion`)
...
- [ ] Offline write-queue in `src/sync.js` (IndexedDB) for ledger/feedback writes when offline — visible sync diagnostics are now shipped, but durable offline persistence/replay still needs a true IndexedDB queue
