...
- [ ] [SIL] Add component tests for ProfitBoost and FirstBet — extracted in S58 alongside BonusBet/KellyCriterion but still lack dedicated test files; vitest JSX infrastructure now in place from S59
- [ ] [SIL] Wire topPlaybook into Studio contract brief followUps — `buildStudioSnapshot` brief object doesn't carry the matched playbook as a structured field; the machine-readable Studio contract should include `topPlaybook` in the brief so downstream consumers (Hub, Social Dashboard) can consume it without re-computing
