...
function MasteryBar({ label, level, reviewPct, accuracy }) {
  ...
  return (
    <div style={{ flex: 1, minWidth: 72 }}>
      ...
      <div style={{ height: 3, background: K.s3, borderRadius: 2, overflow: 'hidden' }}>
        <div style={{ height: 3, borderRadius: 2, background: color, width: `${reviewPct}%`, transition: 'width 0.7s cubic-bezier(0.22,1,0.36,1)' }} />
      </div>
    </div>
  );
}
...
export default function DashboardHero({ totalProfit, openBetsCount, booksComplete, navigate, streak = 0 }) {
  ...
  const reviewDepthBand = mastery?.reviewDepthBand;
  ...
  const activeLanes = mastery
    ? Object.entries(mastery.perType).filter(([, d]) => d.reviews > 0).sort(([, a], [, b]) => b.reviews - a.reviews).slice(0, 4)
    : [];
  const allLanes = mastery ? Object.entries(mastery.perType) : [];
  const weakLane = allLanes.length > 0
    ? allLanes
        .filter(([, d]) => d.reviews < 15)
        .sort(([, a], [, b]) => a.reviews - b.reviews)[0]
    : null;
