...
import { juiceLabel, juiceColor } from "../lib/juiceScore.js";
import { fontD } from "../lib/shared.js";

export default function JuiceScore({ score }) {
  if (score == null || isNaN(score)) return null;
  const color = juiceColor(score);
  const label = juiceLabel(score);
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: `${color}0d`, border: `1px solid ${color}30`, borderRadius: 8, marginTop: 10 }}>
      <div style={{ textAlign: "center", minWidth: 52 }}>
        ...
        <div style={{ fontSize: 8, color, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", marginTop: 1 }}>JUICE</div>
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color, marginBottom: 4 }}>{label} PLAY</div>
        ...
        <div style={{ fontSize: 9, color: "#7a8fa8", marginTop: 3 }}>Promo quality score · 0–100</div>
      </div>
    </div>
  );
}
