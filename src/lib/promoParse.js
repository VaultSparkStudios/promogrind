function parseDollar(value) {
  return Number.parseFloat(String(value || "").replace(/[$,\s]/g, ""));
}
...
function extractAmounts(text) {
  return [...String(text || "").matchAll(/\$\s?(\d[\d,]*(?:\.\d{1,2})?)/g)]
...
function scoreTypeSignals(normalized) {
  return {
...
function calculatorForType(type) {
  return {
    bonus_bet: "bonus-bet",
...
function explanationForType(type, amount, percentage, expiryDays) {
...
function actionForType(type, amount, percentage) {
  switch (type) {
    case "bonus_bet":
