...
const identity = resolveProjectIdentity({
  explicitProject: projectIndex >= 0 ? args[projectIndex + 1] : null,
  root: ROOT,
});
...
if (!identity.ok) {
  const result = { ok: false, decision: 'deny', reason: identity.reason, ...identity, trigger, writeMode };
  console.log(jsonMode ? JSON.stringify(result, null, 2) : `DENY scheduled-write admission: ${result.reason}`);
  process.exit(64);
}
const { project, rootProject } = identity;
const receiptBase = { project, rootProject, projectSource: identity.source, trigger, writeMode };

if (!scheduled) {
  const result = { ok: true, decision: 'allow', reason: `interactive trigger ${trigger}`, ...receiptBase };
