...
- Risks created or removed: removed dependency vulnerabilities and the missing public-repo package-trust fallback; removed a false failing dashboard-smoke artifact parser. Remaining deploy risk is specific: production Supabase `create-checkout` is stale and rejects `scout_monthly`; redeploy attempt failed because `SUPABASE_ACCESS_TOKEN`/Supabase CLI login is unavailable.
- Recommended next move: authenticate Supabase CLI, run `npm run deploy:function:checkout`, rerun Deploy Pages, then continue production auth email, Stripe smoke, and friend-beta proof recording.
