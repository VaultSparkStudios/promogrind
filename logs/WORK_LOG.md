...
- Remote Pages preflights `30327745917` and `30328393421` stopped before artifact creation/deployment because Linux exposed an integration fixture outside the scanner's intended repository boundary. Root-fixed the test with a deterministic full-base64url-alphabet token inside the repo boundary, canonical `fileURLToPath`, and synchronous cleanup; also added a closeout ordering invariant so `PROJECT_STATUS.json` is mirrored and self-checked after its final write. Full `verify:launch-local` returned direct exit 0 after the repairs.
